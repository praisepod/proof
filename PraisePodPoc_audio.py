...
class Camera:
	...
	def RecordAV(self):
			self.MakeFile(self.debug_filename)
			...
			with open("debug.log", "a") as f:
				f.write("Frame Height: " + str(self.frame.height) + "\n")
				f.write("Frame Width: " + str(self.frame.width) + "\n")
				f.write("FPS: " + str(fps) + "\n")
			while True:
				img = cv.QueryFrame(self.capture)
				cv.PutText(self.frame,"PraisePod - Proof Of Concept", (10,frame_size[1] - 30), self.defaultFont, (0,0,255))
				...
				if cv.WaitKey(10) == 32:
					if self.recording == False:
						writer = cv.CreateVideoWriter("temp/" + self.output_filename_base + ".avi", cv.CV_FOURCC('F', 'L', 'V', '1'), fps, frame_size, True)
						self.recording = True;
					else:
						self.recording = False;
